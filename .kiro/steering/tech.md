...
- `extension.js` は VS Code 連携、カスタムエディタ登録、診断、Webview HTML 組み立てを担当する。
...
`lib/ntfYamlModel.js` は共有モデル層である。

- パーサ、シリアライザ、診断、ブロック分類は、Node テストとブラウザ Webview の両方から再利用できるようにする。
...
- 新しい CLI 挙動は解析ロジックを重複させず、共有モデルを再利用する。
- Excel 移行経路では外部変換依存を許容する。ただし、中心となる YAML モデル、検証、差分の挙動はローカルかつ決定的に保つ。
