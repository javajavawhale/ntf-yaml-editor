...
どちらも row 配列形式で扱えるが、NTF の読み取り仕様では固定長ファイルだけフィールド長行を持つ。
ディレクティブ行は固定長ファイル固有の値を持つが、表示、編集、差分の基本仕様は可変長ファイルと同じ表コンポーネントで扱う。

この判断はユーザー確認済みの NTF ドメイン知識を根拠にする。
根拠は、固定長ファイルと可変長ファイルがどちらも NTF のファイル系 row 列として読まれ、固定長だけフィールド長行を追加で持つという確認である。
