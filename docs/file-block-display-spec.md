...
固定長ファイルと可変長ファイルは、どちらも YAML 上では block 配下の row 配列として扱う。
ただし NTF の読み取り仕様では、可変長ファイルは「レコード種別/フィールド名行、データ型行、データ行...」を基本形とし、固定長ファイルはその間にフィールド長行を持つ。
NTF YAML Editor はディレクティブ名・値、データ型、フィールド長の妥当性を検証しない。
...
可変長ファイルとの差分は、NTF が固定長ファイルのフィールド長行を必須の構造行として読む点である。
表示、編集、差分の基本仕様は可変長ファイルと同じである。
エディタはディレクティブ値、データ型、フィールド長の妥当性を検証しない。
...
| `[ "", "半角数字", "半角英字", "全角文字" ]` | データ型行 |
| `[ "", "001", "Tokyo", "base" ]` | データ行 |

固定長ファイルも同じ内部表現を使う。
固定長か可変長かで row 配列という内部形は分けない。
ただし固定長ファイルでは、レコード種別/フィールド名行、データ型行、フィールド長行、データ行の順序を保つ。
...
ただし、ディレクティブ行以外の先頭列は補助セルとして扱う。
`header`、`data`、`データ型`、`フィールド長` などの値が入っている場合も、先頭列は構造セルとしては強調しない。
...
補助セルは、ファイル定義の行構造を読みやすくするためのセルであり、通常データセルとも構造セルとも区別する。
...
| `[ "header", ... ]` | 1列目を補助セル、2列目以降をヘッダ定義として強調する |
| `[ "data", ... ]` | 1列目を補助セル、2列目以降をレコード種別/フィールド名行として強調する |
| `[ "record-length", "12" ]` | ディレクティブ行として表示する。値の妥当性は検証しない |
| `[ "データレコード", "id", "name" ]` | 1列目を補助セル、2列目以降を固定長ファイルのレコード種別/フィールド名行として強調する |
| `[ "データ型", "半角数字", "全角文字" ]` | 1列目を補助セル、2列目以降を固定長ファイルのデータ型行として強調する |
| `[ "フィールド長", "3", "4" ]` | 1列目を補助セル、2列目以降を固定長ファイルのフィールド長行として強調する |
