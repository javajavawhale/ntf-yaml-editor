(function(root, factory) {
  ...
  function diffRawRows(baseBlock, headBlock) {
    ...
    const rows = lcsMatchAndDiff(baseRows, headRows, columns, rawRowsEqual, rawRow => rawRowObject(rawRow, width));
    return { status: rows.length ? "changed" : "unchanged", columns, rows };
  }
  ...
  function diffRow(key, before, after, columns) {
    ...
    const cells = columns.map(column => {
      const beforeRow = rowValue(before);
      const afterRow = rowValue(after);
  ...
  function rowValue(item) {
    if (!item) return null;
    return Object.hasOwn(item, "row") ? item.row : item;
  }
  ...
  // Legacy summary-style HTML renderer. This is intentionally separate from the
  // VS Code cell-diff panel export and is kept as a base for a future one-page
  // aggregate report.
  function renderSummaryHtmlReport(report) {
  ...
  function renderUnifiedDiffContent(report) {
  ...
  function writeSummaryHtmlReport(report, outputFile) {
