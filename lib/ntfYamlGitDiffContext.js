(function(root, factory) {
  ...
  function diffWorkingTreeAllFiles(options) {
    ...
    const diffArgs = (baseIsWorkingTree || baseIsIndex) && (headIsWorkingTree || headIsIndex)
      ? ["diff", "--name-status", "-M", "--", "*.yaml", "*.yml"]
      : baseIsWorkingTree || baseIsIndex
        ? ["diff", "--name-status", "-M", headRef, "--", "*.yaml", "*.yml"]
        : headIsWorkingTree || headIsIndex
          ? ["diff", "--name-status", "-M", baseRef, "--", "*.yaml", "*.yml"]
          : ["diff", "--name-status", "-M", baseRef, headRef, "--", "*.yaml", "*.yml"];
    const diffStatus = git(diffArgs, repositoryPath);
    const untracked = baseIsWorkingTree || headIsWorkingTree || baseIsIndex || headIsIndex
