...
const { suite, teardown, test } = require("mocha");
...
suite("NTF YAML Editor E2E", () => {
  teardown(async () => {
    await closeAllEditors();
  });

  test("activates the extension and registers user-facing commands", async () => {
  ...
  test("renders webview HTML with table editor controls for representative YAML", async () => {
    const extension = vscode.extensions.getExtension(extensionId);
    await extension.activate();
  ...
  test("opens a YAML file with the custom table editor", async () => {
  ...
  test("opens the active text editor through NTF YAML: Open as Table", async () => {
  ...
  test("publishes NTF YAML diagnostics for invalid testShots blocks", async () => {
  ...
  test("round-trips a real fixture through the extension save path without losing critical YAML shape", async () => {
  ...
  test("preserves null sentinel rows when saving a migrated sample fixture", async () => {
